                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-gray-900 rounded-lg p-4">
                                        <div className="text-sm text-gray-300 mb-2">
                                            {syntaxExamples[selectedSyntax].name} ({syntaxExamples[selectedSyntax].extension})
                                        </div>
                                        <div className="text-xs text-gray-400 mb-3">
                                            {syntaxExamples[selectedSyntax].description}
                                        </div>
                                        <pre className="text-sm text-white font-mono overflow-x-auto">
                                            {syntaxExamples[selectedSyntax].example}
                                        </pre>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 p-6 rounded-lg border border-purple-200/50">
                            ...
                            <div className="bg-gray-900 rounded p-4 font-mono text-xs text-white overflow-x-auto">
                                ...
                                <div>  <span className="text-green-400">width</span>: <span className="text-yellow-400">percentage</span>(<span className="text-blue-400">$size</span> / <span className="text-blue-400">$columns</span>);</div>
                                <div>{`}`}</div>
                                <br />
                                <div><span className="text-purple-400">@each</span> <span className="text-blue-400">$breakpoint</span>, <span className="text-blue-400">$screen-size</span> <span className="text-purple-400">in</span> <span className="text-blue-400">$breakpoints</span> {`{`}</div>
                                <div>  <span className="text-purple-400">@for</span> <span className="text-blue-400">$i</span> <span className="text-purple-400">from</span> <span className="text-red-400">1</span> <span className="text-purple-400">through</span> <span className="text-blue-400">$grid-columns</span> {`{`}</div>
                                <div>    <span className="text-yellow-400">.col-{'#{'}$breakpoint{'}'}-{'#{'}$i{'}'}</span> {`{`}</div>
                                <div>      <span className="text-purple-400">@include</span> <span className="text-yellow-400">make-col</span>(<span className="text-blue-400">$i</span>);</div>
                                <div>    {`}`}</div>
                                <div>  {`}`}</div>
                                <div>{`}`}</div>
                            </div>
