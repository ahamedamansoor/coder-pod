  ...
  return res.json();
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-base">
            Run all three styles together and view how comments shape readability.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-lg overflow-hidden border bg-white dark:bg-gray-900">
            <div className="flex items-center justify-between px-4 py-2 text-xs font-medium bg-blue-100 dark:bg-blue-900/30">
              <span className="uppercase tracking-wide text-blue-700 dark:text-blue-300">comments.js</span>
              <span className="text-blue-600/70 dark:text-blue-400/70">Single-line, multi-line, JSDoc</span>
            </div>
            <pre className="font-mono text-xs px-4 py-3 whitespace-pre overflow-x-auto">{commentsJs}</pre>
          </div>

          {onOpenWebPlayground && (
            <Button onClick={() => onOpenWebPlayground(commentsHtml, '', commentsJs)} className="w-full md:w-auto">
