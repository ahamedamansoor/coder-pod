...
export default function MaximumSumSubarrayK() {
  ...
  const getCodeWithValues = (stepData: typeof steps[0]) => {
    return [
      ...
      { line: 13, code: `    windowSum = windowSum - arr[i - k] + arr[i];`, active: stepData.action === 'removing' || stepData.action === 'removed' || stepData.action === 'adding' || stepData.action === 'added', indent: 2, values: stepData.action === 'removing' ? `remove arr[${stepData.removed}] = ${array[stepData.removed]}` : stepData.action === 'removed' ? `${stepData.sum}` : stepData.action === 'adding' ? `add arr[${stepData.added}] = ${array[stepData?.added]}` : stepData.action === 'added' ? `windowSum = ${stepData.sum}` : '' },
