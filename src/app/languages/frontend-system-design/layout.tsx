...
import { FrontendSystemDesignLayoutProvider, useFrontendSystemDesignLayout } from './frontend-system-design-layout-context';
...
function FrontendSystemDesignTopicLayoutContent({ children }: { children: React.ReactNode }) {
  const params = useParams();
  const router = useRouter();
  const { isEditorOpen, setIsEditorOpen } = useFrontendSystemDesignLayout();
  ...
  const language = languages.find((lang) => lang.slug === 'frontend-system-design');
  if (!language) {
    notFound();
  }
  ...
  return (
    <SidebarProvider>
      <div
        id="frontend-system-design-topic-page"
        data-test="frontend-system-design-topic-page"
...
export default function FrontendSystemDesignTopicLayout({
  children,
}: {
  ...
  return (
    <FrontendSystemDesignProvider>
      <FrontendSystemDesignLayoutProvider>
        <FrontendSystemDesignTopicLayoutContent>
          {children}
        </FrontendSystemDesignTopicLayoutContent>
      </FrontendSystemDesignLayoutProvider>
    </FrontendSystemDesignProvider>
