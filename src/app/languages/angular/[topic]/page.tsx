...
import { AngularContentDisplay } from '@/components/languages/angular/angular-content-display';
import { AngularLearningRoadmap } from '@/components/languages/angular/angular-learning-roadmap';
...
function TopicPageContent() {
  ...
  const language: Language | undefined = languages.find((lang) => lang.slug === 'angular');
    ...
    <ResizablePanelGroup direction="horizontal" className="w-full">
      <ResizablePanel defaultSize={100} minSize={30}>
        <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 h-full bg-background">
          {isLearningPlanTopic ? (
            <AngularLearningRoadmap language={language} />
          ) : (
            <AngularContentDisplay
              topic={selectedTopic}
              language={language}
              onOpenEditor={() => {}}
            />
...
export default function AngularTopicPage() {
    return (
