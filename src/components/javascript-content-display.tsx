...
export function JavascriptContentDisplay({ 
  ...
  const { completedTopics, handleToggleComplete } = useJavascript();
  const { user } = useUser();
      ...
      {!isLearningPlanTopic && (
        <Card className="mt-8">
          ...
          <CardContent className="space-y-4">
            <Textarea placeholder={`e.g., "What's the difference between let and const?"`} value={question} onChange={(e) => setQuestion(e.target.value)} disabled={isAsking} />
      ...
      {qaResult && (
        <Card className="border-primary/50 bg-primary/5 animate-in fade-in-50 duration-500">
          <CardHeader className="flex-row items-start gap-4">
            <div className="bg-primary text-primary-foreground p-2 rounded-full"><Sparkles className="w-5 h-5"/></div>
            <div><CardTitle>AI Answer</CardTitle><CardDescription>Here's what our AI assistant came up with.</CardDescription></div>
          </CardHeader>
