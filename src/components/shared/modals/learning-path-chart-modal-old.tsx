...
export const LearningPathChartModal = ({
  ...
  // Only show "View Learning Path" button for these specific roadmaps
  const allowedLearningPaths = ['html', 'css', 'scss', 'tailwind', 'javascript', 'selenium', 'dsa'];
  const showLearningPathButton = allowedLearningPaths.includes(language.slug);
  ...
  // Get all unique categories from both roadmaps
  const allCategories = compareTopicsByCategory ? 
    Array.from(new Set([...categories.map(([cat]) => cat), ...Object.keys(compareTopicsByCategory)])) :
      ...
      <DialogContent showCloseButton={false} className="max-w-full w-screen h-screen p-0 gap-0 overflow-hidden rounded-none">
        ...
        <div className="absolute top-4 left-4 z-50 flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <Select value={compareLanguage} onValueChange={(value) => {
              ...
              <SelectContent>
                {allLanguages
                  .filter(lang => lang.slug !== language.slug)
                  .map(lang => (
        ...
        <div className="relative w-full h-full overflow-auto pt-20 pb-12">
          <div className={cn(
            ...
            {isComparing && (
              <div className="absolute left-3/4 top-0 bottom-0 -translate-x-1/2 w-0.5 bg-purple-500 z-0 hidden md:block" />
            )}
            ...
            <div className="hidden md:block relative z-10 space-y-24">
              {(isComparing ? allCategories : categories.map(([cat]) => cat)).map((category, categoryIndex) => {
                const [categoryName, topics] = isComparing ? 
                  [category as string, topicsByCategory[category] || []] :
                  [category as string, (categories[categoryIndex] as [string, Topic[]])[1]];
                ...
                // Get comparison topics for this category
                const compareTopics = isComparing && compareTopicsByCategory ? 
                  (compareTopicsByCategory[categoryName as string] || []) : [];
                const compareLeftTopics = compareTopics.filter((_, i) => i % 2 === 0);
                const compareRightTopics = compareTopics.filter((_, i) => i % 2 === 1);
                ...
                return (
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      isComparing ? "grid-cols-[1fr_auto_1fr_auto_1fr]" : "grid-cols-[1fr_auto_1fr]"
                    )}>
                      {/* Left Column */}
                          ...
                          {compareLeftTopics.map((topic, i) => {
                            return (
                              ...
                              >
                                ...
                                <div className={cn(
                                  "group relative w-64 p-0 rounded-xl border text-sm font-medium transition-all duration-300 hover:scale-105 hover:-translate-y-1 backdrop-blur-md shadow-lg overflow-hidden",
                                  "bg-purple-50/80 dark:bg-purple-900/40 border-purple-400/50 hover:border-purple-400/70 hover:shadow-xl hover:shadow-purple-500/10"
                                )}>
                                  <div className="p-4">
                                    <div className="mb-1.5 text-sm font-bold line-clamp-1 border-b border-dashed border-purple-200 dark:border-purple-700 pb-2 text-purple-800 dark:text-purple-100">
                                      {topic.title}
                                    </div>
                                    <div className="text-xs leading-relaxed line-clamp-3 text-purple-700/80 dark:text-purple-200/70">
                                      {topic.explanation}
                                    </div>
                                  </div>
                      ...
                      {/* Middle Spine for Comparison */}
                      {isComparing && (
                        <div className="w-px bg-purple-500 mx-4" />
                      )}

                      {/* Comparison Right Column */}
                      {isComparing && (
                        <div className="flex flex-col gap-4 items-start">
                          {compareRightTopics.map((topic, i) => {
                            return (
                              <div
                                key={topic.slug}
                                className="flex items-center animate-slide-right"
