...
export function ReactContentDisplay({ 
  ...
  const { completedTopics, handleToggleComplete } = useReact();
  const { user } = useUser();
      ...
      {!isLearningPlanTopic && (
        <Card className="mt-8">
          ...
          <CardContent className="space-y-4">
            <Textarea placeholder={`e.g., "What's the difference between state and props?"`} value={question} onChange={(e) => setQuestion(e.target.value)} disabled={isAsking} />
