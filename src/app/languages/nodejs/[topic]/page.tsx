...
import { NodejsContentDisplay } from '@/components/languages/nodejs/nodejs-content-display';
...
import { NodejsLearningRoadmap } from '@/components/languages/nodejs/nodejs-learning-roadmap';
import { useNodejsLayout } from '../nodejs-layout-context';
...
function TopicPageContent() {
  ...
  const { isEditorOpen, setIsEditorOpen } = useNodejsLayout();
  ...
  const language: Language | undefined = languages.find((lang) => lang.slug === 'nodejs');
    ...
    <ResizablePanelGroup direction="horizontal" className="w-full">
      <ResizablePanel defaultSize={100} minSize={30}>
        <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 h-full bg-background">
          {isLearningPlanTopic ? (
            <NodejsLearningRoadmap language={language} />
          ) : (
            <NodejsContentDisplay
              topic={selectedTopic}
              language={language}
            />
          )}
...
export default function NodejsTopicPage() {
    return (
