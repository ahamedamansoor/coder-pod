...
import { GitProvider } from './git-context';
import { GitLayoutProvider, useGitLayout } from './git-layout-context';
...
function GitTopicLayoutContent({ children }: { children: React.ReactNode }) {
  const params = useParams();
  const router = useRouter();
  const { isEditorOpen, setIsEditorOpen } = useGitLayout();
  ...
  const language = languages.find((lang) => lang.slug === 'git');
  if (!language) {
    notFound();
  }
...
export default function GitTopicLayout({
  children,
}: {
